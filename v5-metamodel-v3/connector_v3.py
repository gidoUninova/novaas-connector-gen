...
def covertIdB64(id):
    ...
    b64string = encoded_bytes.decode("utf-8")
    return b64string
...
for i in range(0,len(aas_opdata)) :
    ...
    if aas_opdata[i]["modelType"] == "SubmodelElementCollection":
    ...
    if aas_opdata[i]["modelType"] == "BasicEventElement":
...
while len(coll_list) != 0:
    ...
    for i in range(0,len(coll_list)):
        for j in range(0,len(coll_list[i])):
            ...
            if coll_list[i][j]["modelType"] == "SubmodelElementCollection":
                coll_list.append(coll_list[i][j]["value"])
            
            #same as above (check events)
            if coll_list[i][j]["modelType"] == "BasicEventElement":
...
with open(filepath_flow, 'r') as f_flow:
    ...
    flow_name = input("Enter the name for your flow: ")
    flow_data[0]["label"] = flow_name
    ...
    for i in range(0,len(flow_data)):
        ...
        if "type" in flow_data[i].keys() and flow_data[i]["type"] == "tab":
            if flow_data[i]["label"] == "DPDM/OperationalData":
                flow_data[i]["label"] = flow_name
